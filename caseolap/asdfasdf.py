...
class Caseolap(object):
    ...
    def calculate_category2entity2ntf(self):
        ...
        for category, entity2tf in self.category2entity2tf.items():
            ...
            total_hits = self.category2total_tf[category]
            num_entities_found = len([tf for tf in entity2tf.values() if tf > 0])
            
            hit_to_entity_ratio = total_hits/num_entities_found
            ...
            for entity, tf in entity2tf.items():
                ...
                # Calculate
                ntf = (tf*(k1+1))/\
                      (tf+(k1*(1-b+(b*(hit_to_entity_ratio/num_entities_found)))))
    ...
    def calculate_all_distinctiveness_scores(self, dump = False): 
        ...
        ''' Calculate numerator '''
        # Numerator = e^(entity-category ntf/ndf) 
        ...
        for category,ent2exp_ntf_ndf_ratio in category2entity2exp_ntf_ndf_ratio.items():
            ...
            for entity, exp_ntf_ndf_ratio in ent2exp_ntf_ndf_ratio.items():
                ...
                entity2distinctiveness[entity] = exp_ntf_ndf_ratio/denominator
    ...
    def calculate_caseolap_score(self,dump=False):
        '''
        FUNCTION:
        ...
        # Export
        if dump:
            self.df_builder(self.category2entity2caseolap, 'caseolap')
            self.dump_json(self.category2entity2caseolap, 'caseolap')

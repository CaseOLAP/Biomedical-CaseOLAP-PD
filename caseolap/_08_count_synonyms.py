...
class CountSynonyms(object):
    ...
    def count_synonym(self, b_id, batch, key, es, logfile,\
        ...
        with open(temp_pmid_syn_cnt_path,'w') as fout1,\
            ...
            for syn_index, syn in enumerate(batch):
                ...
                # Print progress
                if b_id == 1:
                    print('Batch 1 Progress:', syn_index,'/',len(batch), end='\r')
                ...
                for hit in s.scan():                             
                    ...
                    if pmid in self.relevant_pmids:
                        ...
                        if type(hit.abstract) == str: 
                            hit.abstract = hit.abstract.replace('- ','@$#!')\
                                                       .replace('-',' ')\
                                                       .replace('@$#!','- ').strip()
                        ...
                        # Determine the sections of the publication to search 
                        if key == 'full_text':
                            if type(hit.full_text) == str: 
                                hit.full_text = hit.full_text.replace('- ','@$#!')\
                                                             .replace('-',' ')\
                                                             .replace('@$#!','- ')\
                                                             .strip()
    ...
    def synonym_search(self, key, logfile, syn_pmid_count_in, index_name):          
        ...
        jobs = []
        for b_id, batch in enumerate(batch):
